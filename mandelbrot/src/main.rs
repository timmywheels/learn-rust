...
fn main() {
    ...
    let bounds: (usize, usize) =
        parse_pair(&args[2].to_string(), 'x').expect("Error parsing image dimensions");
...
///
/// Specifically, `s` shouuld have the form <left><separator><right>, where <separator> is
/// the character given by the `separator` argument, and <left> and <right> are both
/// strings that can be parsed b `T::from_str`. `separator` must be an ASCII character.
///
...
/// Parse a pair of floating-point numbers separated vy a comma as a complex number
...
/// and lower-right corners on the pixel buffer
fn render(
    pixels: &mut [u8],
    bounds: (usize, usize),
    upper_left: Complex<f64>,
